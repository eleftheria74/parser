...
describe("scoreImageUrlUrl(url)", () => {
  it("gets 20 points for a positive lead img hint", () => {
    ...
    import_assert.default.equal(scoreImageUrl(url), 20);
  });
  ...
  it("loses 10 points for a gif", () => {
    ...
    import_assert.default.equal(scoreImageUrl(url), -10);
    const url2 = "http://example.com/foogif/bar";
    import_assert.default.equal(scoreImageUrl(url2), 0);
  });
  it("gains 10 points for a jpg", () => {
    const url = "http://example.com/foo/bar.jpg";
    import_assert.default.equal(scoreImageUrl(url), 10);
    const url2 = "http://example.com/foo/bar.jpeg";
    import_assert.default.equal(scoreImageUrl(url2), 10);
    const url3 = "http://example.com/foojpg/bar";
    import_assert.default.equal(scoreImageUrl(url3), 0);
    const url4 = "http://example.com/foo.jpg?bar=baz";
    import_assert.default.equal(scoreImageUrl(url4), 10);
  });
});
...
describe("scoreByParents($img)", () => {
  it("gets 25 points if it has a figure parent", () => {
    ...
    `);
    const $img = $("img").first();
    import_assert.default.equal(scoreByParents($img), 25);
  });
  it("gets 0 points if the img has no figure parent", () => {
    const $ = import_cheerio.default.load("<div><img /></div>");
    const $img = $("img").first();
    import_assert.default.equal(scoreByParents($img), 0);
  });
  it("gets 15 points if parent or gparent has photo hints", () => {
    ...
    `);
    const $img = $("img").first();
    import_assert.default.equal(scoreByParents($img), 15);
  });
});
describe("scoreBySibling($img)", () => {
  it("gets 25 points if its sibling is figcaption", () => {
    ...
    `);
    const $img = $("img").first();
    import_assert.default.equal(scoreBySibling($img), 25);
  });
  it("gets 15 points if its sibling has photo hints", () => {
    ...
    `);
    const $img = $("img").first();
    import_assert.default.equal(scoreBySibling($img), 15);
  });
});
describe("scoreByDimensions($img)", () => {
  it("penalizes skinny images", () => {
    ...
    `);
    const $img = $("img").first();
    import_assert.default.equal(scoreByDimensions($img), -50);
  });
  ...
  it("ignores sprites", () => {
    ...
    `);
    const $img = $("img").first();
    import_assert.default.equal(scoreByDimensions($img), 0);
  });
  it("penalizes images with small areas", () => {
    ...
    `);
    const $img = $("img").first();
    import_assert.default.equal(scoreByDimensions($img), -100);
  });
  it("prefers the largest images", () => {
    ...
    `);
    const $img = $("img").first();
    import_assert.default.equal(scoreByDimensions($img), 1e3);
  });
});
describe("scoreByPosition($imgs, index)", () => {
  it("gives higher scores to images that come first", () => {
    ...
    `);
    const $imgs = $("img");
    import_assert.default.equal(scoreByPosition($imgs, 0), 3);
  });
});
