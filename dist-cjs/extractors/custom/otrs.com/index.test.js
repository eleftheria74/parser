...
describe("OtrsComExtractor", () => {
  describe("initial test case", () => {
    ...
    it("returns the title", async () => {
      ...
      import_assert.default.equal(title, `Silver Society \u2013 The young old people`);
    });
    it("returns the author", async () => {
      const { author } = await result;
      import_assert.default.equal(author, "Andr\xE9 Mindermann");
    });
    it("returns the date_published", async () => {
      const { date_published } = await result;
      import_assert.default.equal(date_published, `2019-03-18T09:00:11.000Z`);
    });
