...
describe("ScienceflyComExtractor", () => {
  describe("initial test case", () => {
    ...
    let url;
    beforeAll(() => {
      url = "http://sciencefly.com/video-shows-false-killer-whale-snagging-tuna-bait/";
    ...
    it("returns the title", async () => {
      ...
      import_assert.default.equal(title, "Video shows false killer whale snagging tuna bait");
    });
    it("returns the author", async () => {
      const { author } = await result;
      import_assert.default.equal(author, "Paul");
    });
    it("returns the date_published", async () => {
      const { date_published } = await result;
      import_assert.default.equal(date_published, "2016-12-23T16:44:36.000Z");
    });
