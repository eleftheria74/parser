...
describe("BlisterreviewComExtractor", () => {
  describe("initial test case", () => {
    ...
    it("returns the title", async () => {
      ...
      import_assert.default.equal(title, `2021-2022 Line Outline`);
    });
    it("returns the author", async () => {
      const { author } = await result;
      import_assert.default.equal(author, `Luke Koppa`);
    });
    it("returns the date_published", async () => {
      const { date_published } = await result;
      import_assert.default.equal(date_published, `2019-04-09T14:00:15.000Z`);
    });
