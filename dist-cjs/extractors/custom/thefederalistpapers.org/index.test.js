...
describe("ThefederalistpapersOrgExtractor", () => {
  describe("initial test case", () => {
    ...
    let url;
    beforeAll(() => {
      url = "http://thefederalistpapers.org/education-2/the-failure-of-public-schooling-in-one-chart";
    ...
    it("returns the title", async () => {
      ...
      import_assert.default.equal(title, "The FAILURE Of Public Schooling In One Chart");
    });
    it("returns the author", async () => {
      const { author } = await result;
      import_assert.default.equal(author, "Bushrod Washington");
    });
    it("returns the date_published", async () => {
      const { date_published } = await result;
      import_assert.default.equal(date_published, "2016-12-21T16:31:34.000Z");
    });
