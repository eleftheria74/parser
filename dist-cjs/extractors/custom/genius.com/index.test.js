...
describe("GeniusComExtractor", () => {
  describe("initial test case", () => {
    ...
    it("returns the title", async () => {
      ...
      import_assert.default.equal(title, `Purple Rain`);
    });
    it("returns the author", async () => {
      const { author } = await result;
      import_assert.default.equal(author, "Prince and the Revolution");
    });
    it("returns the date_published", async () => {
      const { date_published } = await result;
      const newDatePublished = (0, import_moment.default)(date_published).format();
      import_assert.default.equal(newDatePublished.split("T")[0], "1984-06-25");
    });
