...
describe("ApartmentTherapyExtractor", () => {
  describe("initial test case", () => {
    ...
    it("returns the title", async () => {
      ...
      import_assert.default.equal(title, "A Light Filled LA Loft");
    });
    it("returns the author", async () => {
      const { author } = await result;
      import_assert.default.equal(author, "Apartment Therapy Submissions");
    });
    it("returns the date_published", async () => {
      const { date_published } = await result;
      import_assert.default.equal(date_published, "2016-10-13T21:00:00.000Z");
    });
