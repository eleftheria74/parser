...
var import_clean = __toESM(require("./clean"));
describe("clean($)", () => {
  it("removes script elements", () => {
    const html = "<div><script>alert('hi')</script></div>";
    const $ = import_cheerio.default.load(html);
    import_assert.default.equal((0, import_clean.default)($).html(), "<div></div>");
  });
  ...
  it("removes comments", () => {
    ...
    const $ = import_cheerio.default.load(html);
    import_assert.default.equal((0, import_clean.default)($).html(), "<div>HI </div>");
  });
});
